...
ARKitでは、カメラ画像に含まれる視覚情報と、デバイスの慣性センサーによるモーション情報を統合した 
...
カメラ座標系からARKit座標系への回転行列、
$\mathbf{t}_{\mathrm{cam}\rightarrow\mathrm{AR}}$ は
ARKit座標系におけるカメラ原点位置を表す並進ベクトルである。
...
得られるため、ARKitによるカメラ位置姿勢により、適宜自己位置を更新する。
...
\subsection{実行結果}
上記の理論に基づいてカメラ位置を更新した結果を図\ref{three}に示す。
\begin{figure}[H]
    \centering
    \begin{tabular}{ccc}
        \includegraphics[width=0.3\linewidth]{figures/3_1.jpg} &
        \includegraphics[width=0.3\linewidth]{figures/3_2.jpg} &
        \includegraphics[width=0.3\linewidth]{figures/3_3.jpg} \\
        目的地指定 & カメラ位置(1) & カメラ位置(2)
    \end{tabular}
    \caption{目的地に対するオブジェクト描画結果}
    \label{three}
\end{figure}

ユーザの世界座標は，東向きが $X$ 正，北向きが $Y$ 正，高さ方向が $Z$ 正となるように定義されている。
ここで，$C_{imu}$ は ARKit 座標系におけるカメラ位置を，ユーザの世界座標系へ変換したものである。
図\ref{three}より，ユーザが目的地方向（東向き）へ移動した際に，$C_{imu}$ の $X$ 成分が $25.53→27.85$ と正方向に変化していることが確認できる。
また，$Y$ および $Z$ 成分についても，想定どおり更新されていることが確認された。
