...
def estimateFNS(data):
    ...
    while True:
        ...
        for i in range(dataMod.shape[0]):
            ...
            alp = np.dot(myu.T, np.dot(covs[i], myu)).item()  # スカラー値を抽出
            weight[i, 0] = 1 / (alp)
...
def calcRepLSMandMSE():
    ...
    plt.legend()
    ax.yaxis.set_major_formatter(ptick.ScalarFormatter(useMathText=True))   # y軸の表記変更
    ax.ticklabel_format(style="sci", axis="y", scilimits=(-1,-6))   # 指数表記(10^-6)
    plt.grid() #グリッド
