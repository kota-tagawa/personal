...
\begin{table}[H]
  ...
  \begin{tabular}{l|l}
    ...
    項目 & 内容 \\
    \hline \hline
    全方位カメラ & RICOH THETA 360 \\
    全方位カメラ解像度 & 11K（$11008 \times 5504$ 画素）\\
    全方位カメラ焦点距離 & 約1752 \\
    \hline
    OS & Windows 11 \\
...
各全方位カメラについては、2方向以上の透視投影画像を画角 $90 \times 90$ 画素で生成し、
図\ref{five:three}に示すように透視投影画像上の2次元座標と世界座標の3次元座標を対応付けることで、全方位カメラの位置と姿勢を推定した。
すべての全方位カメラにおいて、推定位置と実測位置のずれが 10\,cm 未満であることを確認している。
...
\begin{figure}[H]
  ...
  \end{tabular}
  \caption{ワイヤーフレームモデル（サンプリング間隔：左から 4\,m, 3\,m, 2\,m）}
  \label{five:five}
\end{figure}
...
動画の解像度は$1920 \times 1080$ピクセルであり、これを1fps (frames per second)の間隔で静止画として切り出し、評価用画像セットとした。
...
一般に屋内では \texttt{indoor} モデルが推奨されるが、本研究の対象である大学構内の廊下においては、以下の3点の理由から屋外モデルの方が適していると判断した。
...
\begin{figure}[H]
  \centering
  \begin{tabular}{cc}
    \includegraphics[height=3cm]{figures/5/sift1.jpg}&
    \includegraphics[height=3cm]{figures/5/sift2.jpg}\\
    \includegraphics[height=3cm]{figures/5/akaze1.jpg}&
    \includegraphics[height=3cm]{figures/5/akaze2.jpg}\\
    \includegraphics[height=3cm]{figures/5/deep1.jpg}&
    \includegraphics[height=3cm]{figures/5/deep2.jpg}\\
  \end{tabular}
...
しかし、本実験では評価のためにすべてのテクスチャに対して特徴点マッチングを行っているが、実際のナビゲーション運用時においては、
...
右図(b)は、特徴点マッチングが一見正しく行われているにもかかわらず、正確な位置姿勢が得られなかった事例である。 
本事例では、壁面の法線に対してカメラの光軸が大きな角度をなす、いわゆる斜め方向からの観測となっている。 
このような条件下では、透視投影による歪みが顕著となり、
画像上のわずかな座標ズレが3次元空間における大きな位置誤差へと増幅される性質がある。
その結果、正面からの観測時と比較して、同程度のマッチング精度が得られていたとしても、最終的な位置姿勢の推定結果には大きな偏差が生じたと考えられる。
ただし、斜め方向から観測している場合でも正しく推定できる事例も確認されており、
推定失敗に至る決定的な条件の特定には至っていない。
...
\begin{enumerate}
    ...
    \item \textbf{リカバリ処理} \\
    PCサーバによる位置推定が20秒間連続して成功しなかった場合、トラッキングが喪失したと判定し、再度初期位置推定処理を実行する。
\end{enumerate}

PCサーバ側で行う2回目以降の自己位置推定においては、計算効率と精度向上のため、
モバイル端末から得られる直前の推定位置および視線ベクトルを事前情報として利用する。
この際、誤マッチングによる外れ値を排除するため、許容誤差の閾値を位置については0.5\,m、姿勢については10$^{\circ}$と設定した。
...
第一に、画像データのPCサーバへの送信、計算処理、そして結果の受信を含む一連の処理に平均して約0.5秒の遅延が生じ、
歩行者の移動速度に対して自己位置の更新が追いついていないためである。
第二に、移動中に生じる手ブレや、壁面の特徴点が少ない区間においてマッチング数が不足し、解が得られないフレームが発生したためである。
結果として、ナビゲーションに必要なリアルタイム性が確保できず、ユーザへの経路提示が遅れる場面が散見された。
...
本手法では、VIOが継続的に相対移動量を推定することで、画像マッチングの通信遅延や計算失敗より推定が途切れた区間を埋め、
...
\subsection{アプリケーションの実行の様子} 
実装した屋内ナビゲーションアプリケーションの実行時の様子を図\ref{five:fifteen}に示す。 
図(a)は目的地を選択するUI画面、図(b)および(c)はAR空間上に重畳表示された誘導矢印と目的地を示すピン、図(d)は目的地到着時の判定画面である。 
これらの表示により、ユーザの現在地から目的地までの経路情報が視覚的に提示され、適切にナビゲーション機能が動作していることが確認できる。

\begin{figure}[H] 
  \centering 
  \begin{minipage}[b]{0.48\linewidth} 
    \centering 
    \includegraphics[width=\linewidth]{figures/5/result1.jpg} 
    \\ (a) 目的地選択画面 
  \end{minipage} 
  \hfill 
  \begin{minipage}[b]{0.48\linewidth} 
    \centering 
    \includegraphics[width=\linewidth]{figures/5/result2.jpg} 
    \\ (b) 誘導矢印、目的地ピンの表示１
  \end{minipage}
  ...
  \begin{minipage}[b]{0.48\linewidth} 
    \centering 
    \includegraphics[width=\linewidth]{figures/5/result3.jpg} 
    \\ (c) 誘導矢印、目的地ピンの表示２
  \end{minipage} 
  \hfill 
  \begin{minipage}[b]{0.48\linewidth} 
    \centering 
    \includegraphics[width=\linewidth]{figures/5/result4.jpg} 
    \\ (d) 目的地到着時の表示 
  \end{minipage} 
  \caption{屋内アプリケーションの実行画面} 
  \label{five:fifteen} 
\end{figure}