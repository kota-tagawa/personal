...
さらに、屋内環境ではテクスチャの乏しい壁面や照明変化の影響により、常に十分な特徴量が得られるとは限らず、
...
そのため、本研究では提案する自己位置推定手法をナビゲーションの「絶対的な位置補正」の基準情報として用いつつ、
...
一方で、長時間動作させた際の累積誤差（ドリフト）や、カメラ視野が環境特徴に乏しい状況（白い壁のみが映る場合など）では推定精度が低下する可能性がある点に注意が必要である\cite{AppleARKitWorldTracking}。

ARKitでは、ワールド座標系が内部的に定義されており、セッション初期化時の端末位置が原点として設定される。
座標軸は右手系で定義されており、一般に重力と反対方向（垂直上向き）を$Y$軸とし、
...
図\ref{four:two}にその定義を示す。

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.4\textwidth]{figures/4/arkit_camaxis.png}
...
具体的には、ARKitのカメラ座標 $\bm{X}_{\mathrm{camAR}}$ と、本研究で用いるOpenCVのカメラ座標 $\bm{X}_{\mathrm{camPC}}$ の関係は、
...
高負荷な自己位置推定処理を担うPCサーバーによって構成される。
...
その結果がモバイル端末へフィードバックされることで、端末内部のAR座標系（VIO）と世界座標系の間で整合性が取られ、
正確なルート案内とAR表示が可能となる。以下の節で、各処理の詳細について述べる。
...
\begin{equation}
  ...
  \bm{X}_{\mathrm{camAR}}
  +
  \bm{t}_{\mathrm{cam}\rightarrow\mathrm{AR}}
  \label{eq:one}
\end{equation}
...
\begin{equation}
  \bm{X}_{\mathrm{camPC}}
  =
  \bm{R}_{\mathrm{world}\rightarrow\mathrm{cam}}
  \bm{X}_{\mathrm{world}}
  +
  \bm{t}_{\mathrm{world}\rightarrow\mathrm{cam}}
  \label{eq:two}
\end{equation}
...
\begin{align}
  \bm{X}_{\mathrm{AR}}
  &=
  \bm{R}_{\mathrm{cam}\rightarrow\mathrm{AR}}
  \left(
    \bm{S} (
      \bm{R}_{\mathrm{world}\rightarrow\mathrm{cam}}
      \bm{X}_{\mathrm{world}}
...
まず、マップから取得した経路上の目的地座標を世界座標系で取得し、前節で求めた変換式を用いてAR座標系への変換を行うことで、AR空間上における目的地の描画位置を算出する。
